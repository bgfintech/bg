...
<html lang="en">
    ...
    <body padding-top:0.08rem;>
		...
		<div class="bank_card">
		  ...
		  <%
			Object ot = request.getAttribute("treatyid");
			if(ot==null || "".equals((String)ot)){			
		 %>
		 ...
		 <div class="bank_card_num clearfix">
		    <span class="bank_card_numL">银行卡号</span>
		    <span class="bank_card_numR">${trans.BANKCARDNO }</span>
		    <input type="hidden" id="bankcardno" value="${trans.BANKCARDNO }"/>
		    <!--<i class="error"></i>-->
		  </div>		  
		 <%
			}
		...
		<%
		 ...
		 %> 
		 	<input type="button" onclick="ps();" class="quit_btn confirBtn" value="确定">
		  <%
			}
		 %>
		</div>
...
function paysubmit(){
	...
	$.ajax({
            type: "POST",
            dataType: "json",
            url:"transController.do?gbptreatyconfirmpay",
            data: {
                openid:openid,
               	pbankphone:pbankphone,
               	smscode:smscode,
               	orderno:orderno,
               	name:name,
               	bankno:bankno,
            ...
            success: function(data) {                   
                if(data.SUCCESS == "00"){
                   alert("支付已成功");
                   window.location.href="acctController.do?gotreatyquickpay&openid="+openid;
                }else{
                   alert(data.FAIL);
                }
                pay=0;
